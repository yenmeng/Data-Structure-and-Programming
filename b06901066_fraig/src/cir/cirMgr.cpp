...
CirMgr::readCircuit(const string& fileName)
{
    ...
    for(unsigned i=0;i<I;i++){
        ...
        PIlist.push_back(_gatelist[literalID/2]);
    }
    //****** L ******
    string line2;
    //****** O ******
    ...
    for(unsigned i=0;i<O;i++){
        ...
        _gatelist[M+i+1]=out;
        POlist.push_back(_gatelist[M+i+1]);
    ...
    for(unsigned i=0;i<A;i++){
        ...
        (RHS1%2==0)?_gatelist[RHS1/2]->fanoutID.push_back(2*(LHS/2)):_gatelist[RHS1/2]->fanoutID.push_back(2*(LHS/2)+1);
        (RHS2%2==0)?_gatelist[RHS2/2]->fanoutID.push_back(2*(LHS/2)):_gatelist[RHS2/2]->fanoutID.push_back(2*(LHS/2)+1);
...
CirMgr::printFECPairs() const
{
    for(size_t i=0;i<FEClist.size();++i){
        ...
        for(unsigned j=0;j<getGate(FEClist[i])->fec.size(); ++j){
            cout<<' '<<((getGate(FEClist[i])->fec[j]%2==0)? "":"!")
            <<getGate(FEClist[i])->fec[j]/2;
        }
        cout << endl;
    }
...
{
    for(size_t i=0;i<gate->faninlist.size();++i)
    {
        if(!gate->faninlist[i]->isGlobalRef() &&
           gate->faninlist[i]->getTypeStr() != "UNDEF" ){
            DFStraversal(gate->faninlist[i]);
        }
    }
...
CirMgr::writeAag(ostream& outfile) const
{
    size_t dfs_A=0;
    for(size_t i=0;i<DFSlist.size();++i){
        if(DFSlist[i]->getTypeStr()=="AIG") ++dfs_A;
    }
    ...
    outfile<<"NTUEE DSnP fall,2018"<<endl;
}
