...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
       MemRecycleList<T>* recycle = &_recycleList[m];
       if(recycle->getArrSize()==n) return recycle;
       else{
           while(recycle->getNextList()!=0){
               recycle=recycle->getNextList();
               if(recycle->getArrSize()==n) return recycle;
           }
       }
       MemRecycleList<T>* create = new  MemRecycleList<T>(n);
       recycle->setNextList(create);
       recycle=recycle->getNextList();
       return recycle;
   }
   ...
   T* getMem(size_t t) {
      ...
      // 4. Get the memory from _activeBlock
           ...
           if(_activeBlock->getMem(t,ret)) return ret;
       }
      ...
      //    Print this message for memTest.debug
       ...
       if(remain>=S){
           ...
           rn=getArraySize(remain);
           _activeBlock->getMem(remain,ret);
   ...
   size_t getNumBlocks() const {
      // TODO
      size_t cnt = 1;
      MemBlock<T>* n=_activeBlock;
      while(n->getNextBlock())
      {
          cnt++;
          n=n->getNextBlock();
      }
