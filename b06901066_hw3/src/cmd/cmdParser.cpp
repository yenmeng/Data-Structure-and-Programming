...
CmdParser::parseCmd(string& option)
{
   ...
   string str = _history.back();
    ...
    string cmd=str.substr(pos,pos+endpos);
    CmdExec* e=getCmd(cmd);
    if(e!=0) {
        if((pos+endpos)!=string::npos) {option=str.substr(pos+endpos);}
        return e;
    }
    else{
   ...
   // TODO...
    ...
    if(str.find_first_not_of(' ')== string::npos)
    {
        ...
        for(CmdMap::const_iterator it = _cmdMap.begin(); it!=_cmdMap.end(); ++it){
            ...
            if(cnt%5==0 && it!=_cmdMap.end()) cout<<endl;
        }
        reprintCmd();
    }
    // 2.3.4. LIST ALL PARTIALLiY MATCHED/SINGLY MATCHED/NO MATCHED COMMAND
    if(str.find_first_not_of(' ')!= string::npos)
    {
	...
	else s=str.substr(str.find_first_not_of(' '));

        for(CmdMap::const_iterator it = _cmdMap.begin(); it!=_cmdMap.end(); ++it)
	{		
		int m=it->first.size();
		string full_cmd=it->first+it->second->getOptCmd();
		
		if(s.size()>=1){
			if(myStrNCmp(full_cmd, s, 1) == 0)
			{
				if(s.size()>=m){marked=true;}
				string match=it->first+it->second->getOptCmd();	
				matched.push_back(match);
			}
		}	
	}
	if(matched.size()==1){
		if(pos==string::npos && str.back()!=' ' && s.size()<matched[0].size()+1){
			for(int i=s.size();i<matched[0].size();i++)
                	{insertChar(matched[0][i],1);}
                	insertChar(' ',1);
			_tabPressCount=0; return;
		}
		else if(marked==true) {
			if(_tabPressCount==1){
                	cout<<endl;
                	getCmd(matched[0])->usage(cout);
			reprintCmd(); return;
			}	
			else if (_tabPressCount>1 && str.back()==' '){
			vector<string> files;
			listDir(files,"",".");
			if(files.size()!=0){
			int cnt=0;
			int min_strlen=files[0].size();
			for(int i=1;i<files.size();i++)
			{
				if(files[i].size()>=min_strlen)
				min_strlen=files[i].size();
			}
			for(int j=1;j<files.size();j++)
			{
				...
				{
					if(files[0][i]==files[j][i]) n++;
					else{same_char=false; break;}
				}
				if(j==1) {cnt=n;}
				else if(cnt>n) {cnt=n;}
			}
			...
			for(int i=0;i<files.size();i++)
			{
		     		cout << setw(16) << left << files[i];
                     		if((i+1)%5==0 && (i+1)!=files.size()) cout<<endl;
			...
			listDir(files,t,".");
			if(files.size()==1) {
			for(int i=t.size();i<files[0].size();i++){insertChar(files[0][i],1);}
			insertChar(' ',1);
			mybeep();
			return;
			}
   	...
   	else if (matched.size()>1 && pos==string::npos){
		cout<<endl;
		for(int i=0;i<matched.size();i++)
		{
		     cout << setw(12) << left << matched[i];
 	 	     if((i+1)%5==0 && (i+1)!=matched.size()) cout<<endl;
    		}
		reprintCmd();	
	}
	else mybeep();
