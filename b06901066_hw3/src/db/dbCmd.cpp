...
DBAppendCmd::exec(const string& option)
{
   ...
   // check option
    ...
    if(options.size()==2){
        ...
        string _key=options[0];
        int _val;
        if(!myStr2Int(options[1],_val))
            return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[1]);

        DBJsonElem j(_key,_val);
        if(!dbjson.add(j)){
        cerr<<"Error: Element with key \""<<j.key()<<"\" already exists!!"<<endl;
            return CMD_EXEC_ERROR;
       }
    }
...
{  
   // TODO...
    string key;
    if (!CmdExec::lexSingleOption(option, key, true))
	 return CMD_EXEC_ERROR;
    if(!dbjson.isread) {
	    cerr<<"Error: DB not created yet!!"<<endl;
	     return CMD_EXEC_ERROR;
	}
    if (key.size() == 0)
    {
